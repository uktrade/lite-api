...
class OrganisationExternalLocationsTests(DataTestClient):
    def setUp(self):
        ...
        self.external_location = self.create_external_location(
            name="storage facility", org=self.organisation
        )
        self.url = reverse(
            "organisations:external_locations", kwargs={"org_pk": self.organisation.pk}
        )
    ...
    def test_site_list(self):
        ...
        self.assertEqual(
            response_data["external_locations"][0]["name"], self.external_location.name
        )
