...
class GovUserViewTests(DataTestClient):
    ...
    def test_get_individual_gov_user(self):
        ...
        self.assertEqual(
            len(response_data["gov_users"]), self.gov_user_preexisting_count + 1
        )

    def test_filter_users(self):
        response = self.client.get(
            reverse("gov_users:gov_users") + "?teams=" + str(self.team.id),
            **self.gov_headers
        )
        response_data = response.json()
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            len(response_data["gov_users"]), self.gov_user_preexisting_count
        )
