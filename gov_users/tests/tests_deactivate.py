...
class GovUserDeactivateTests(DataTestClient):
    ...
    def test_deactivate_a_user(self):
        ...
        url = reverse('gov_users:gov_user', kwargs={'pk': user_2.id})
        response = self.client.put(url, data, format='json', **self.gov_headers)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual((GovUser.objects.get(id=user_2.id).status), GovUserStatuses.DEACTIVATED)
    ...
    def test_deactivate_and_reactivate_a_user(self):
        ...
        response = self.client.get(reverse('gov_users:gov_users'), **{'HTTP_GOV_USER_EMAIL': str(user_2.email)})
