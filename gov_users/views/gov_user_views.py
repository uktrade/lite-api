...
class GovUserList(APIView):
    ...
    def get(self, request):
        ...
        if teams:
            gov_users = GovUser.objects.filter(team__id__in=teams.split(",")).order_by(
                "email"
            )
        else:
    ...
    @swagger_auto_schema(
        request_body=GovUserCreateSerializer, responses={400: "JSON parse error"}
    )
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return JsonResponse(
                data={"gov_user": serializer.data}, status=status.HTTP_201_CREATED
            )
...
class GovUserDetail(APIView):
    ...
    @swagger_auto_schema(
        request_body=GovUserCreateSerializer, responses={400: "Bad Request"}
    )
    def put(self, request, pk):
        ...
        with reversion.create_revision():
            ...
            if serializer.is_valid():
                ...
                return JsonResponse(
                    data={"gov_user": serializer.data}, status=status.HTTP_200_OK
                )
