...
class CreateCaseAdviceTests(DataTestClient):
    ...
    @staticmethod
    def create_advice_base_test(test_client, advice_type, url):
        data = {
            'text': 'I Am Easy to Find',
            'note': 'I Am Easy to Find',
            'type': advice_type,
            'end_user': str(test_client.standard_application.end_user.id),
        }
        ...
        response = test_client.client.post(url, **test_client.gov_headers, data=[data])
        response_data = response.json()['advice'][0]

        test_client.assertEqual(response.status_code, status.HTTP_201_CREATED)

        test_client.assertEqual(response_data['text'], data['text'])
        test_client.assertEqual(response_data['note'], data['note'])
        test_client.assertEqual(response_data['type']['key'], data['type'])
        test_client.assertEqual(response_data['end_user'], data['end_user'])

        advice_object = Advice.objects.get()

        # Ensure that proviso details aren't added unless the type sent is PROVISO
        if advice_type != AdviceType.PROVISO:
            test_client.assertTrue('proviso' not in response_data)
            test_client.assertEqual(advice_object.proviso, None)
        else:
            test_client.assertEqual(response_data['proviso'], data['proviso'])
            test_client.assertEqual(advice_object.proviso, data['proviso'])

        # Ensure that refusal details aren't added unless the type sent is REFUSE
        if advice_type != AdviceType.REFUSE:
            test_client.assertTrue('denial_reasons' not in response_data)
            test_client.assertEqual(advice_object.denial_reasons.count(), 0)
        else:
            test_client.assertEqual(response_data['denial_reasons'], data['denial_reasons'])
            test_client.assertEqual(convert_queryset_to_str(advice_object.denial_reasons.values_list('id', flat=True)),
                             data['denial_reasons'])
    ...
    def test_create_end_user_case_advice(self, advice_type):
        ...
        """
        self.create_advice_base_test(self, advice_type, self.standard_case_url)
