...
class CreateGoodsCountriesDecisions(DataTestClient):
    def setUp(self):
        ...
        self.goods_countries_url = reverse(
            "cases:goods_countries_decisions", kwargs={"pk": self.case.id}
        )
    ...
    def test_make_goods_countries_decisions_success(self):
        ...
        self.assertEqual(
            GoodCountryDecision.objects.count(), len(data["good_countries"])
        )
        self.assertEqual(len(response.json()["data"]), len(data["good_countries"]))

    def test_saving_overwrites_previous_assignment(self):
        self.create_good_country_decision(
            self.case, self.goods_type_1, self.country_1, "approve"
        )
