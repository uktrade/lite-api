...
class ControlListClassificationsList(APIView):
    ...
    def post(self, request):
        ...
        return JsonResponse(
            data={"id": clc_query.id, "case_id": clc_query.case.get().id},
        ...
        with reversion.create_revision():
            if clc_good_serializer.is_valid():
                if "validate_only" not in data or data["validate_only"] == "False":
                    ...
                    CaseActivity.create(
                        activity_type=CaseActivityType.CLC_RESPONSE,
                        case=query.case.get(),
                        user=request.user,
                    )
                    ...
                    for (
                        user_relationship
                    ) in UserOrganisationRelationship.objects.filter(
                    ...
                    return JsonResponse(
                        data={
                            "control_list_classification_query": clc_good_serializer.data
                        }
                    )
                else:
                    return JsonResponse(
                        data={"control_list_classification_query": data},
                        status=status.HTTP_200_OK,
                    )

            return JsonResponse(
                data={"errors": clc_good_serializer.errors},
