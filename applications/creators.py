...
def check_party_document(party, is_mandatory):
    ...
    try:
        ...
        document = None
        if is_mandatory:
            return getattr(applications.Standard, f"NO_{party.type.upper()}_DOCUMENT_SET")

    if document:
        if document.safe is None:
            return getattr(applications.Standard, f"{party.type.upper()}_DOCUMENT_PROCESSING")
        elif not document.safe:
            return getattr(applications.Standard, f"{party.type.upper()}_DOCUMENT_INFECTED")
        else:
            return None
...
def _validate_goods_types(draft, errors):
    ...
    if not results:
        errors["goods"] = applications.Open.NO_GOODS_SET
...
def _validate_standard_licence(draft, errors):
    ...
    if not GoodOnApplication.objects.filter(application=draft):
        errors["goods"] = applications.Standard.NO_GOODS_SET
    ...
    if ultimate_end_user_required:
        if len(draft.ultimate_end_users.values_list()) == 0:
            errors["ultimate_end_users"] = applications.Standard.NO_ULTIMATE_END_USERS_SET
        else:
...
def _validate_open_licence(draft, errors):
    if len(CountryOnApplication.objects.filter(application=draft)) == 0:
        errors["countries"] = applications.Open.NO_COUNTRIES_SET
...
def validate_application_ready_for_submission(application):
    ...
    if (
        ...
        errors["location"] = applications.Generic.NO_LOCATION_SET
