    ...
    def post(self, request, application):
        ...
        if not location_ids:
            return JsonResponse(data={
                'errors': {'external_locations': ['You have to pick at least one external location']}
        ...
        if application.status and application.status.status in get_read_only_case_statuses():
            ...
            if not application.status or application.status.status == CaseStatusEnum.APPLICANT_EDITING:
                new_locations = [get_location(location_id, request.user.organisation)
                                 for location_id in location_ids if location_id not in previous_location_ids]
            else:
        ...
        if application.status and application.status.status in get_read_only_case_statuses():
            ...
            if application.status and application.status.status != CaseStatusEnum.APPLICANT_EDITING:
                if ExternalLocationOnApplication.objects.filter(application=application).count() == 1:
                    return JsonResponse(data={'error': "Go back and change your answer from ‘Change a site, or delete "
