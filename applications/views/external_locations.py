...
class ApplicationExternalLocations(APIView):
    ...
    def get(self, request, application):
        external_locations_ids = ExternalLocationOnApplication.objects.filter(
            application=application
        ).values_list("external_location", flat=True)
        external_locations = ExternalLocation.objects.filter(
            id__in=external_locations_ids
        )
        serializer = ExternalLocationSerializer(external_locations, many=True)

        return JsonResponse(
            data={"external_locations": serializer.data}, status=status.HTTP_200_OK
        )
    ...
    def post(self, request, application):
        ...
        if not location_ids:
            return JsonResponse(
                data={
                    "errors": {
                        "external_locations": [
                            "You have to pick at least one external location"
                        ]
                    }
                },
        ...
        previous_location_ids = [
            ...
            for previous_location_id in previous_locations.values_list(
                "external_location__id", flat=True
            )
        ]
        ...
        if (
            ...
            for location_id in location_ids:
                ...
                if (
                    previous_location_countries
                    and new_location.country.id not in previous_location_countries
                ):
                    return JsonResponse(
                        data={
                            "errors": {
        ...
        if data.get("method") != "append_location":
            ...
            removed_locations = previous_locations.filter(
                external_location__id__in=removed_location_ids
            )
        ...
        for new_location in new_locations:
            serializer = ExternalLocationOnApplicationSerializer(
                data={
                    "external_location": new_location.id,
        ...
        set_external_location_case_activity(
            removed_sites, removed_locations, new_locations, request.user, application
        )
        ...
        return JsonResponse(
            data={"external_locations": response_data}, status=status.HTTP_201_CREATED
        )
...
class ApplicationRemoveExternalLocation(APIView):
    ...
    def delete(self, request, application, ext_loc_pk):
        if application.status and application.status.status in get_case_statuses(
            ...
            return JsonResponse(
                data={
                    "error": f"Application status {application.status.status} is read-only."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        set_external_location_case_activity(
            None, removed_locations, None, request.user, application
        )
        removed_locations.delete()

        return JsonResponse(
            data={"success": "External location deleted"},
