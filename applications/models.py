...
class SiteOnApplication(models.Model):
    ...
    site = models.ForeignKey(
        Site, related_name="sites_on_application", on_delete=models.CASCADE
    )
    application = models.ForeignKey(
        BaseApplication, related_name="application_sites", on_delete=models.CASCADE
    )
...
class ApplicationDenialReason(models.Model):
    ...
    reason_details = models.TextField(
        default=None, blank=True, null=True, max_length=2200
    )
...
class StandardApplication(BaseApplication):
    ...
    ultimate_end_users = models.ManyToManyField(
        UltimateEndUser, related_name="application_ultimate_end_users"
    )
    ...
    third_parties = models.ManyToManyField(
        ThirdParty, related_name="application_third_parties"
    )
...
class GoodOnApplication(models.Model):
    ...
    good = models.ForeignKey(
        Good, related_name="goods_on_application", on_delete=models.CASCADE
    )
    application = models.ForeignKey(
        StandardApplication, related_name="goods", on_delete=models.CASCADE
    )
...
class CountryOnApplication(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    application = models.ForeignKey(
        OpenApplication, related_name="application_countries", on_delete=models.CASCADE
    )
    country = models.ForeignKey(
        Country, related_name="countries_on_application", on_delete=models.CASCADE
    )
