...
class RemovingGoodsOffDraftsTests(DataTestClient):
    ...
    def test_remove_a_good_from_application_success_when_good_is_on_multiple_applications(
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            GoodOnApplication.objects.filter(application=application1).count(), 0
        )
        self.assertEqual(
            GoodOnApplication.objects.filter(application=application2).count(), 1
        )
        self.assertEqual(
    ...
    def test_delete_good_from_application_in_an_editable_status_success(
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            GoodOnApplication.objects.filter(application=application).count(), 0
        )
    ...
    def test_delete_good_from_application_in_read_only_status_failure(
        ...
        self.assertEqual(
            GoodOnApplication.objects.filter(application=application).count(), 1
        )
