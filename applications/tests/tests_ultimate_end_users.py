...
class ApplicationUltimateEndUserTests(DataTestClient):
    ...
    def test_submit_draft_with_ultimate_end_users_success(self):
        ultimate_end_user = self.create_ultimate_end_user('ultimate end user', self.draft, self.organisation)
        data = {'id': self.draft.id}
        response = self.client.post(self.url, data, **self.exporter_headers)

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        self.assertEqual(UltimateEndUser.objects.get(application_id=self.draft.id), ultimate_end_user)
