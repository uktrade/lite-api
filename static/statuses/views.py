...
class StatusesAsList(APIView):
    def get(self, request):
        ...
        return JsonResponse(data={"statuses": serializer.data}, status=status.HTTP_200_OK)
...
class StatusProperties(APIView):
    def get(self, request, status):
        ...
        return JsonResponse(
            data={"is_read_only": status_properties[0], "is_terminal": status_properties[1]}, status=status.HTTP_200_OK
        )
