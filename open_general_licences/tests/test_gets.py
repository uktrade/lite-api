...
class ExporterListTests(DataTestClient):
    def setUp(self):
        ...
        self.open_general_licence_case = OpenGeneralLicenceCaseFactory(
            open_general_licence=self.ogl_1,
            site=self.organisation.primary_site,
            organisation=self.organisation,
        )

    def test_exporter_view_site_licences_success(self):
        response = self.client.get(self.url + "?site=" + str(self.organisation.primary_site.id), **self.exporter_user)

        self.assertEquals(response.status_code, status.HTTP_200_OK)

    def test_exporter_view_active_licences_success(self):
        response = self.client.get(self.url + "?active_only=True", **self.exporter_user)

        self.assertEquals(response.status_code, status.HTTP_200_OK)

    def test_exporter_view_registered_licences_success(self):
        response = self.client.get(self.url + "?registered=True", **self.exporter_user)
