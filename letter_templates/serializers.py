...
class LetterTemplateSerializer(serializers.ModelSerializer):
    name = serializers.CharField(
        ...
        validators=[
            UniqueValidator(
                queryset=LetterTemplate.objects.all(), lookup="iexact", message=LetterTemplates.UNIQUE_NAME,
            )
        ],
        error_messages={"blank": LetterTemplates.NAME_REQUIRED},
    )
    ...
    case_types = PrimaryKeyRelatedSerializerField(
        ...
        error_messages={"required": LetterTemplates.SELECT_THE_CASE_TYPES},
        many=True,
    )
    ...
    layout = PrimaryKeyRelatedSerializerField(
        ...
        error_messages={"required": LetterTemplates.SELECT_THE_LAYOUT},
    )
    ...
    def validate_case_types(attrs):
        if not attrs:
            raise serializers.ValidationError(LetterTemplates.NEED_AT_LEAST_ONE_CASE_TYPE)
        return attrs

    @staticmethod
    def validate_letter_paragraphs(attrs):
        if not attrs:
            raise serializers.ValidationError(LetterTemplates.NEED_AT_LEAST_ONE_PARAGRAPH)
        return attrs
