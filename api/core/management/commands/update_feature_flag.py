...
class Command(BaseCommand):
    def add_arguments(self, parser: CommandParser) -> None:
        parser.add_argument("name", nargs="?", help="The name of the flag.")
        parser.add_argument(
            "-l", "--list", action="store_true", dest="list_flags", default=False, help="List existing samples."
        )
        parser.add_argument("--everyone", action="store_true", dest="everyone", help="Activate flag for all users.")
        parser.add_argument(
            "--deactivate", action="store_false", dest="everyone", help="Deactivate flag for all users."
        )
        parser.add_argument(
            "--percent",
            "-p",
        ...
        parser.add_argument(
            "--staff", action="store_true", dest="staff", default=False, help="Turn on the flag for Django staff."
        )
        parser.add_argument(
            "--authenticated",
            action="store_true",
        ...
        parser.add_argument(
            ...
            action="append",
            default=list(),
            help="Turn on the flag for the listed organisation primary keys.",
        )
        parser.add_argument(
            "--user",
            "-u",
        ...
        parser.add_argument(
            "--append", action="store_true", dest="append", default=False, help="Append only mode when adding groups."
        )
        parser.add_argument(
            "--rollout", "-r", action="store_true", dest="rollout", default=False, help="Turn on rollout mode."
        )
        parser.add_argument(
            "--testing",
            "-t",
            ...
            dest="testing",
            default=False,
            help="Turn on testing mode, allowing the flag to be specified via " "a querystring parameter.",
        )
        parser.add_argument(
            "--create", action="store_true", dest="create", default=False, help="If the flag doesn't exist, create it."
        )
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if groups := options.pop("group"):
            ...
            if not options_append:
                flag.groups.clear()
            self.stdout.write("Setting group(s): %s" % ([name for name, _id in group_hash.items()]))
            ...
            for username in users:
                try:
                    user_instance = UserModel.objects.get(
                        Q(**{UserModel.USERNAME_FIELD: username}) | Q(**{UserModel.EMAIL_FIELD: username})
                    )
        ...
        if organisations := options.pop("organisations"):
            ...
            for organisation in organisations:
                try:
                    organisation_instance = Organisation.objects.get(pk=organisation)
                    organisations_hash[organisation_instance.name] = organisation_instance.id
                except Organisation.DoesNotExist:
                    raise CommandError(f"Group {organisation} does not exist")
            # If 'append' was not passed, we clear related organisations
            if not options_append:
                flag.organisations.clear()
            self.stdout.write("Setting organisation(s): %s" % ([name for name, _id in organisations_hash.items()]))
            for organisation_id in organisations_hash.values():
                flag.organisations.add(organisation_id)
    ...
    def log_flag_to_stdout(self, flag: AbstractBaseFlag) -> None:
        ...
        self.stdout.write("GROUPS: {}".format(list(flag.groups.values_list("name", flat=True))))
        self.stdout.write("USERS: {}".format(list(flag.users.values_list(UserModel.USERNAME_FIELD, flat=True))))
        self.stdout.write("")
