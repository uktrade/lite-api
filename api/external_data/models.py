...
class NewDenial(TimestampableModel):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False, db_index=True)
    created_by = models.ForeignKey(GovUser, related_name="newdenials_created", on_delete=models.DO_NOTHING, null=True)
    reference = models.TextField(help_text="The reference assigned by the notifying government")
...
class DenialEntity(TimestampableModel):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False, db_index=True)
    denial_id = models.ForeignKey(
        NewDenial,
        related_name="denial_entity",
        on_delete=models.CASCADE,
    )
    type = models.TextField()
    name = models.TextField(help_text="The name of the individual/organization being denied", null=True)
    address = models.TextField(help_text="The address of the individual/organization being denied", null=True)
