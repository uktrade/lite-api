...
from api.applications.tests.factories import CryptoOIELFactory, GoodOnApplicationFactory, StandardApplicationFactory
...
class CryptoOpenLicenceFactory(factory.django.DjangoModelFactory):
    case = factory.SubFactory(CryptoOIELFactory)
    start_date = timezone.now().date()
    duration = DefaultDuration.PERMANENT_OPEN.value
    hmrc_integration_sent_at = None
