...
def populate_good_details(cases: List[Dict]):
...
def populate_denials(cases: List[Dict]):
    case_map = {case["id"]: case for case in cases}
    doas = (
...
def populate_ecju_queries(cases: List[Dict]):
    case_map = {case["id"]: case for case in cases}
    queries = (
    ...
    for query in queries:
        ...
        if case.get("ecju_queries"):
            case["ecju_queries"].append(query)
        else:
            case["ecju_queries"] = [query]
    return list(case_map.values())
