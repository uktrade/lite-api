...
class CaseDetailSerializer(serializers.ModelSerializer):
        ...
        for detail in details:
            for case_queue in qs:
                if case_queue.queue_id == detail["id"]:
                    detail["days_on_queue_elapsed"] = (
                        datetime.now(tz(settings.TIME_ZONE)) - case_queue.created_at
                    ).days
                    break
        return details
    ...
    def get_total_days_elapsed(self, instance):
        return (datetime.now(tz(settings.TIME_ZONE)) - instance.submitted_at).days
