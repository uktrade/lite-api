...
from api.cases.signals import case_pre_save_handler
...
class TestSignals(DataTestClient):
    ...
    def test_case_pre_save_handler_status_changed(self, mocked_flagging_func):
    ...
    @override_settings(FEATURE_C5_ROUTING_ENABLED=True)
    @mock.patch("api.cases.signals.apply_flagging_rules_to_case")
    def test_case_pre_save_handler_status_not_changed(self, mocked_flagging_func):
        submitted = CaseStatus.objects.get(status="submitted")
        case = CaseFactory(status=submitted)
        case_pre_save_handler(Case, case)
    ...
    @override_settings(FEATURE_C5_ROUTING_ENABLED=True)
    @mock.patch("api.cases.signals.apply_flagging_rules_to_case")
    def test_case_pre_save_handler_status_draft(self, mocked_flagging_func):
        submitted = CaseStatus.objects.get(status="submitted")
        case = CaseFactory(status=submitted)
    ...
    def test_case_pre_save_handler_raw(self, mocked_flagging_func):
        ...
        case = CaseFactory(status=submitted)
        case.status = CaseStatus.objects.get(status="initial_checks")
        case_pre_save_handler(Case, case, raw=True)
        assert not mocked_flagging_func.called

    @override_settings(FEATURE_C5_ROUTING_ENABLED=False)
    @mock.patch("api.cases.signals.apply_flagging_rules_to_case")
    def test_case_pre_save_handler_no_flag(self, mocked_flagging_func):
        submitted = CaseStatus.objects.get(status="submitted")
        case = CaseFactory(status=submitted)
        case.status = CaseStatus.objects.get(status="initial_checks")
        case_pre_save_handler(Case, case)
    ...
    @override_settings(FEATURE_C5_ROUTING_ENABLED=True)
    @mock.patch("api.cases.signals.apply_flagging_rules_to_case")
    def test_case_pre_save_handler_signal_fires(self, mocked_flagging_func):
        submitted = CaseStatus.objects.get(status="submitted")
