...
from api.data_workspace.serializers import AuditMoveCaseSerializer, AuditUpdatedStatusSerializer
...
class AuditUpdatedStatusListView(viewsets.ReadOnlyModelViewSet):
    """Expose 'updated status' audit events to data workspace."""

    authentication_classes = (DataWorkspaceOnlyAuthentication,)
    serializer_class = AuditUpdatedStatusSerializer
    pagination_class = LimitOffsetPagination

    def get_queryset(self):
        return Audit.objects.filter(verb=AuditType.UPDATED_STATUS)
