...
def scan_document_for_viruses(self, document_id):
    ...
    with transaction.atomic():
        logging.info("Fetching document %s", document_id)

        document = Document.objects.select_for_update(nowait=True).get(id=document_id)
        if document.virus_scanned_at:
            logging.info("Document %s has already been scanned; safe=%s", document_id, document.safe)
            return

        try:
            document.scan_for_viruses()
        except Exception as exc:  # noqa
            logging.exception("Document virus scan failed")
            raise
