...
class PicklistsViews(DataTestClient):
    ...
    def setUp(self):
        ...
        self.picklist_item_1 = self.create_picklist_item(
            "#1", self.team, PicklistType.PROVISO, PickListStatus.ACTIVE
        )
        self.picklist_item_2 = self.create_picklist_item(
            "#2", self.team, PicklistType.PROVISO, PickListStatus.ACTIVE
        )
        self.create_picklist_item(
            "#3", self.team, PicklistType.REPORT_SUMMARY, PickListStatus.ACTIVE
        )
        self.create_picklist_item(
            "#4", self.team, PicklistType.REPORT_SUMMARY, PickListStatus.DEACTIVATED
        )
        self.create_picklist_item(
            "#5", other_team, PicklistType.ECJU, PickListStatus.ACTIVE
        )

    def test_gov_user_can_see_all_their_teams_picklist_items(self):
        response = self.client.get(
            self.url + "?show_deactivated=True", **self.gov_headers
        )
    ...
    def test_gov_user_can_see_all_their_teams_picklist_items_excluding_deactivated(
        ...
        response = self.client.get(
            self.url + "?show_deactivated=False", **self.gov_headers
        )
        response_data = response.json()
    ...
    def test_gov_user_can_see_filtered_picklist_items_excluding_deactivated(self):
        response = self.client.get(
            self.url + "?type=" + PicklistType.REPORT_SUMMARY, **self.gov_headers
        )
        response_data = response.json()
