...
def initialize(apps, schema_editor):
    ...
    if not GovUser.objects.all():
        for email in json.loads(env("SEED_USERS")):
            gov_user = GovUser(email=email, team=Team.objects.get())
            gov_user.save()

    Role = apps.get_model("users", "Role")
    if not Role.objects.all():
        role = Role(id="00000000-0000-0000-0000-000000000001", name="Default")
        role.save()

    Permission = apps.get_model("users", "Permission")
    if not Permission.objects.all():
        permission = Permission(id="MAKE_FINAL_DECISIONS", name="Make final decisions")
        permission.save()
